...
pub struct StreamerButton{
    ...
    radius: f32,
    border_width: f32,
    on_drop: Box<dyn Fn() -> String>,
    offset: [f32; 2],
    name: String,
}

pub struct PluginButton<Message>{
    width: f32,
    height: f32,
    data: String,
...
impl<Message> PluginButton<Message> {
...
impl<Message, Renderer> Widget<Message, Renderer> for PluginButton<Message>
where
    Renderer: renderer::Renderer + iced_native::text::Renderer,
    Message: Clone 
{
    ...
    fn on_event(
        ...
        update(event, layout, cursor_position, shell, &self.on_drag, &self.on_release, || tree.state.downcast_mut::<PluginButtonState>())
    }
}
...
impl<'a, Message, Renderer> From<PluginButton<Message>> for Element<'a, Message, Renderer>
where
    Renderer: renderer::Renderer + iced_native::text::Renderer,
    Message: Clone + 'a,
{
    fn from(circle: PluginButton<Message>) -> Self {
        Self::new(circle)
    }
}
